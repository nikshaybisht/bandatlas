...
export function ShareCard({ compound, technique, caption }: Props) {
  ...
  const downloadPng = () => {
    ...
    ctx.fillText('BandAtlas  |  figure card', 48, 64)
    ...
    ctx.fillText('github.com/nikshaybisht/bandatlas', 48, h - 44)

    const a = document.createElement('a')
    a.download = `bandatlas-${compound.id}-${technique}.png`
  ...
  const copySummary = async () => {
    const text = [
      ...
      caption,
      compound.cas ? `CAS ${compound.cas}` : '',
      'BandAtlas · github.com/nikshaybisht/bandatlas',
    ]
      .filter(Boolean)
      .join('\n')
    try {
