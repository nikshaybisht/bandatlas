// keep in sync with tools/export-contracts-impl.mjs (node tests import the mjs twin)
...
export function labNoteMarkdown(opts: any) {
  ...
  const lambda =
    spectrum?.lambda_max_nm && spectrum.lambda_max_nm.length
      ? spectrum.lambda_max_nm.map((n: number) => `${n} nm`).join(', ')
      : 'n/a'
  const peaks =
...
export function clipboardMarkdownCitation(opts: any) {
  const { compound, spectrum, technique, url } = opts
  const appVersion = opts.appVersion || '0.0.0'
...
export function compoundBibtex(compound: any, opts: any = {}) {
  const appVersion = opts.appVersion || '0.0.0'
...
export function spectrumSourceBibtex(compound: any, spectrum: any, opts: any = {}) {
